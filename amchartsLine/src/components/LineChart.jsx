...
const LineChart = (props) => {
  ...
  useLayoutEffect(() => {
    ...
    const series = chart.series.push(
      am5xy.LineSeries.new(root, {
        ...
        yAxis: yAxis,
        valueYField: "price",
        fill: am5.color(0x3fd1b9),
        stroke: am5.color(0x089981),
    ...
    const filteredData = mappedPrices.filter((_, index) => {
        return index % 24 === 0 || index === mappedPrices.length - 1;
      });
    const formattedData = filteredData.map(item => ({
        timestamp: new Date(item.timestamp).toISOString().slice(0, 10),
        price: item.price
    }));
  ...
  return <div id="chartdiv" style={{ width: "100%", height: "800px" }} ref={chartRef}></div>;
};
